...
x=max(1,round(rect(1))); %change to int，col
y=max(1,round(rect(2))); %row
w=round(rect(3));%col range
h=round(rect(4));%row range
x_=min(x+w-1,size(I,2));
y_=min(y+h-1,size(I,1));
...
function J = luminance_change(I, option, value)
    ...
    if option == 'c'
        ...
        J(1:mid_r, 1:mid_c, :) = I(1:mid_r, 1:mid_c, :) * value;
        J(mid_r+1:end, mid_c+1:end, :) = I(mid_r+1:end, mid_c+1:end, :) * value;
    end
    if option == 'b'
        % right upper(1:mid_r, mid_c+1:end); left bottom(mid_r+1:end, 1:mid_c)
